...
- course_info：課程基本資訊、上課時間、教室、學分、教學內容、成績評定、課程比較、硬度/甜度詢問、推薦課程。（⚠️如果是問「我禮拜二空堂有什麼推薦的課」，歸類為 course_info，絕不可歸類為 personal_schedule！）
- professor_info：教授專長、聯絡方式(Gmail/Email)、實驗室規定、研究方向、指導條件。
- policy_rules：畢業門檻、選課規定、請假規則、系所規章、雙主修條件。
- calendar_action：行事曆操作（加入/修改/刪除行程、提醒）。只要提到「行事曆」、「加到日曆」就是此類。
- academic_calendar：學校行事曆日期查詢（開學日、期中考、停修期限、連假等）。
- personal_schedule：純粹查詢使用者「自己已經選上的課表」（如：「我今天有什麼課」）。
- personal_transcript：查詢使用者「自己的成績或已修習學分」（如：「我過了幾學分」）。
- chitchat：純粹打招呼、閒聊、抱怨，且完全沒有上述實質查詢需求。
...
3. 【跨域學習與職涯探索】：若使用者在問「我想學程式」、「未來想走前端」、「怎麼準備就業」，這屬於職涯與跨域探索。你【必須】將 `is_career_planning` 設為 true，並且【絕對清空】`academic_year`、`semester`。
   - ⚡ 跨域特判：如果使用者的科系（如企管系）明顯與想學的技能（如寫程式）無關，你【必須一併清空】`dept_short`，讓系統去全校（或資工系）撈課！
4. 【通識課程】：詢問通識時，`dept_short` 設為 `通識中心`，清空 `grade`。可設定 `ge_domain`（人文/社會/自然）。
5. 【防止幻覺】：未明確指定的欄位，請直接在 JSON 中設為 `null`（不要加雙引號！）或直接省略！嚴禁腦補發明內容。
...
def route_and_rewrite(
    ...
    try:
        ...
        # Python 層級保底：如果問課程，且沒有指定老師/教室/系所
        if final_type in ("course_info", "schedule"):
            if "teacher" not in merged_filters and "classroom" not in merged_filters and "dept_short" not in merged_filters and user_profile:
