...
⚠️⚠️ 實體提取核心鐵律 (Strict Entity Extraction Rule) ⚠️⚠️
- 你提取的所有 `metadata_filters` 實體（包括 `teacher`、`course_name_keyword`、`classroom` 等），**必須 100% 僅從【最新提問】（User Question）中提取**。
- **絕對禁止**將未出現在【最新提問】中的任何歷史討論對象（如歷史提到的老師姓名、無關課程名）塞入 `metadata_filters` 條件中！對話歷史僅供理解語境與代名詞指代使用。
...
4. 【跨域學習與職涯探索】：
   - 當使用者在問「我想學程式」、「未來想走前端」、「怎麼準備就業」，或詢問「推薦課程給我」、「有什麼好課」、「有沒有甜課/涼課推薦」等「全校性、非指定科系」的泛化探索問題時，這屬於大範圍探索。你【必須】將 `is_career_planning` 設為 true，並【絕對清空】academic_year、semester、grade、dept_short、required_or_elective。
   - ⚠️【特例保護 - 指定科系推薦】：如果問題明確指定了特定科系（例如：「長照系推薦課程」、「資工系有什麼推薦選修」），這屬於【常規科系課程檢索】，**絕對不可**將 `is_career_planning` 設為 true（必須設為 false）！且**必須保留** `dept_short` 過濾條件（例如填入「長照系」或「資工系」），以防跨系撈取無關資料！
   - ⚡ 跨域特判：若使用者的科系（如企管系）明顯與想學的技能（如寫程式）無關，且沒有指定要查哪科系，你【必須一併清空】`dept_short`，讓系統去全校（或資工系）撈課。
5. 【通識課程】：詢問通識時，`dept_short` 設為 `通識中心`，清空 `grade`。可設定 `ge_domain`（人文/社會/自然）。
6. 【體育課程】：一般學生詢問體育課程或體育課推薦時（意指全校共同體育課，而非特定科系如運休系的專業體育課），`dept_short` 設為 `日大學體育`，清空 `grade`。
7. 【防止幻覺】：未明確指定的欄位，請直接在 JSON 中設為 `null`（不要加雙引號！）或直接省略！嚴禁腦補發明內容。
...
def _extract_metadata_by_rules(question: str) -> dict:
    ...
    # 從 config.DEPT_REGISTRY 動態收集別名與關鍵字
    _dept_patterns = {}
    for k, info in config.DEPT_REGISTRY.items():
        short_name = info["short_name"]
        full_name = info["full_name"]
        _dept_patterns[short_name] = short_name
        _dept_patterns[full_name] = short_name
        for alias in info.get("aliases", []):
            _dept_patterns[alias] = short_name
        for kw in info.get("keywords", []):
            _dept_patterns[kw] = short_name

    # 先檢查是否包含碩士關鍵字
    is_master = any(kw in question for kw in ["碩", "研究所"])
    # 用最長匹配優先（避免「工管」被「工」先匹配到），多科系收集不再單一匹配即 break
    ...
    for kw in sorted_dept_keys:
        if kw in temp_q:
            ...
            final_dept = dept.replace("系", "碩") if (is_master and "系" in dept) else (dept + "碩" if is_master else dept)
            if final_dept not in detected_depts:
                detected_depts.append(final_dept)
            # 刪除已匹配的關鍵字，防重複/子字串匹配
            temp_q = temp_q.replace(kw, "")
            
    if detected_depts:
        if len(detected_depts) == 1:
            filters["dept_short"] = detected_depts[0]
        else:
            filters["dept_short"] = detected_depts
    ...
    if "grade" not in filters and "dept_short" in filters:
        dept_val = filters["dept_short"]
        dept_list = dept_val if isinstance(dept_val, list) else [dept_val]
        for dept_item in dept_list:
            if not isinstance(dept_item, str):
                continue
            short_name = dept_item.replace("系", "").replace("碩", "")
            for g in ["一", "二", "三", "四", "五"]:
    ...
    # ── 體育課程偵測（自動設定 dept_short 為 "日大學體育"）──
    if "體育" in question and not any(kw in question for kw in ["運休", "運動與休閒", "體育系", "體育學系"]):
        filters["dept_short"] = "日大學體育"
        # 體育不分年級，移除 grade 限制
        filters.pop("grade", None)
    ...
    detected_teachers = []
    temp_q_teacher = clean_q
    for teacher in sorted(_known_teachers, key=len, reverse=True):
        if teacher in temp_q_teacher:
            detected_teachers.append(teacher)
            # 防重複/子字串匹配
            temp_q_teacher = temp_q_teacher.replace(teacher, "")
            
    if detected_teachers:
        filters["teacher"] = detected_teachers if len(detected_teachers) > 1 else detected_teachers[0]

    # ── 課程名稱關鍵字（動態清單，從索引自動建構）──
    detected_courses = []
    temp_q_course = question
    for kw in sorted(_known_courses, key=len, reverse=True):
        if kw in temp_q_course:
            detected_courses.append(kw)
            # 防重複/子字串匹配
            temp_q_course = temp_q_course.replace(kw, "")
            
    if detected_courses:
        filters["course_name_keyword"] = detected_courses if len(detected_courses) > 1 else detected_courses[0]
...
async def route_and_rewrite(
    ...
    try:
        ...
        # 統一整合至 gemini_client 的非同步呼叫入口
        from llm.gemini_client import a_call_gemini_json
        data = await a_call_gemini_json(
            prompt=prompt,
            schema=COMBINED_OUTPUT_SCHEMA,
            model="fast",
            thinking="low",
            max_tokens=config.GEMINI_FLASH_MAX_TOKENS,
            timeout=config.GEMINI_FLASH_TIMEOUT,
        )
        
        logger.info(f"合併 Router+Rewrite 非同步回應資料：{data}")
        ...
        _TEMPORAL_KEYS = {"semester", "academic_year", "grade", "dept_short"}
        ...
        # 若 LLM 把課程關鍵字（例如「嵌入式系統概論」）誤認並塞進了 teacher 欄位
        # 我們應該將其從 teacher 中剔除，並安全地轉移/歸併到 course_name_keyword 欄位中
        if "teacher" in llm_filters:
            t_val = llm_filters["teacher"]
            t_list = t_val if isinstance(t_val, list) else [t_val]
            real_teachers = []
            misplaced_courses = []
            for t in t_list:
                if not isinstance(t, str):
                    continue
                # 如果這個名字出現在已知課程名冊（_known_courses）中，或者名字長度大於 4 且包含 "系統", "概論", "原理", "實驗", "微處理器", "程式設計" 等，顯然是課程名稱而非人名
                is_course = t in _known_courses
                is_course = is_course or any(k in t for k in ["系統", "概論", "原理", "實驗", "微處理器", "程式設計", "微算機"])
                is_course = is_course and t not in _known_teachers
                
                if is_course:
                    misplaced_courses.append(t)
                    logger.warning(f"  🛑 偵測到教師與課程名稱混淆：將課程名 '{t}' 自 teacher 移至 course_name_keyword 欄位")
                else:
                    real_teachers.append(t)
            
            if real_teachers:
                llm_filters["teacher"] = real_teachers
            else:
                llm_filters.pop("teacher", None)
                
            if misplaced_courses:
                if "course_name_keyword" not in llm_filters:
                    llm_filters["course_name_keyword"] = []
                elif not isinstance(llm_filters["course_name_keyword"], list):
                    llm_filters["course_name_keyword"] = [llm_filters["course_name_keyword"]]
                    
                for mc in misplaced_courses:
                    if mc not in llm_filters["course_name_keyword"]:
                        llm_filters["course_name_keyword"].append(mc)
        ...
        if "teacher" in llm_filters:
            ...
            for t in t_list:
                if isinstance(t, str):
                    ...
                    # 防呆：台灣老師名字不可能包含數字。如果包含，通常是 LLM 把教室代碼（如 E320 或 324）誤認為老師了！
                    # （注意：外籍老師會有純英文字母，因此只封殺數字）
                    if re.search(r'\d', t):
                        logger.warning(f"  🛑 攔截到 LLM 幻覺：將包含數字的字串 '{t}' 誤認為老師，已自動移除。")
                        continue
                    if t:
                        cleaned_teachers.append(t)
            if cleaned_teachers:
                llm_filters["teacher"] = cleaned_teachers
            else:
                llm_filters.pop("teacher", None)
        ...
        for k, v in rule_filters.items():
            # 【顯式空值保護】若該欄位在 LLM 中已被標記為 _EXPLICIT_NULL，規則引擎絕對不可覆蓋它
            if merged_filters.get(k) == _EXPLICIT_NULL:
                continue
            # 針對容易受「空格/錯字」影響的動態實體（課程、教師）與語意時間，優先信任 LLM 判斷
            if k in ("course_name_keyword", "teacher", "semester", "academic_year") and merged_filters.get(k):
                continue
            merged_filters[k] = v

        # ── 🛡️ 實體存在性安全閥 (Entity Existence Safety Valve) ──
        # 由於對話歷史的干擾，LLM 有時會把歷史中討論的老師、課程關鍵字誤當成當前問題的 filters 提取出來。
        # 如果提取出的 teacher、course_name_keyword 或 classroom 根本沒有出現在使用者最新提問 (question) 中，
        # 我們強制將其剔除，以防止因錯誤硬過濾導致的檢索結果落空。
        if "teacher" in merged_filters:
            t_val = merged_filters["teacher"]
            t_list = t_val if isinstance(t_val, list) else [t_val]
            valid_teachers = []
            for t in t_list:
                if isinstance(t, str):
                    if t in question or any(alias in question for alias in [t + "老師", t + "教授"]):
                        valid_teachers.append(t)
                    else:
                        logger.warning(f"  🛑 實體存在性安全閥攔截：教師 '{t}' 未出現在最新提問中，已自動剔除防污染。")
            if valid_teachers:
                merged_filters["teacher"] = valid_teachers if isinstance(t_val, list) else valid_teachers[0]
            else:
                merged_filters.pop("teacher", None)

        if "course_name_keyword" in merged_filters:
            c_val = merged_filters["course_name_keyword"]
            ...
            for c in c_list:
                if isinstance(c, str):
                    if c in question:
                        ...
                        logger.warning(f"  🛑 實體存在性安全閥攔截：課程關鍵字 '{c}' 未出現在最新提問中，已自動剔除防污染。")
            if valid_courses:
                merged_filters["course_name_keyword"] = valid_courses if isinstance(c_val, list) else valid_courses[0]
            else:
                merged_filters.pop("course_name_keyword", None)

        if "classroom" in merged_filters:
            c_room = merged_filters["classroom"]
            if isinstance(c_room, str) and c_room not in question:
                logger.warning(f"  🛑 實體存在性安全閥攔截：教室 '{c_room}' 未出現在最新提問中，已自動剔除防污染。")
                merged_filters.pop("classroom", None)
        ...
        # 【關鍵修正】教師搜尋豁免年級與科系：
        # 當使用者指定了特定老師，系統應該讓他查到該老師在所有年級與科系的開課狀況。
        if "teacher" in merged_filters:
            removed_grade = merged_filters.pop("grade", None)
            removed_dept = merged_filters.pop("dept_short", None)
            if removed_grade or removed_dept:
                logger.info(f"  🛡️ 教師搜尋安全閥：已自動移除年級與科系限制（指定教師時應全面放行）")

        # 【第二代多聯查】多課程跨學期旁路安全閥 (Multi-Course Temporal Bypass)：
        # 若同時查詢多門課程（course_name_keyword 包含 2 個或以上元素），
        # 主動卸除學期 (semester) 與學年 (academic_year) 限制，以防不同學期的課被誤殺。
        ckw = merged_filters.get("course_name_keyword", [])
        if isinstance(ckw, list) and len(ckw) >= 2:
            removed_year = merged_filters.get("academic_year")
            removed_sem = merged_filters.get("semester")
            merged_filters["academic_year"] = _EXPLICIT_NULL
            merged_filters["semester"] = _EXPLICIT_NULL
            if removed_year or removed_sem:
                logger.info(f"  ⚡ 雙重/多重課程聯查旁路安全閥觸發：已自動將學年與學期設為顯式空值保護，避免跨學期課程被誤殺")

        # 【多科系聯查旁路安全閥 (Multi-Department Bypass)】
        # 當同時查詢/比較多個科系（如資工與電機），或者 dept_short 包含多個科系時，
        # 主動將科系 (dept_short) 設為 _EXPLICIT_NULL 保護罩，並卸除年級限制，以全面放行跨科系的語意檢索。
        dept_val = merged_filters.get("dept_short")
        rule_dept = rule_filters.get("dept_short")
        is_multi_dept = (isinstance(dept_val, list) and len(dept_val) >= 2) or (isinstance(rule_dept, list) and len(rule_dept) >= 2)
        
        if is_multi_dept:
            merged_filters["dept_short"] = _EXPLICIT_NULL
            merged_filters.pop("grade", None)
            logger.info("  ⚡ 多科系聯查旁路安全閥觸發：已自動將科系設為顯式空值保護，並卸除年級限制，全面放行跨科系語意檢索")

        # 判斷是否為「職涯規劃 / 未來大範圍探索 / 跨時空學習路徑」問題
        is_career_planning = bool(data.get("is_career_planning", False))
        
        # 🛡️ 指定科系推薦安全閥：如果過濾器中已提取出明確科系 (dept_short)，
        # 或者是使用者問題中包含任何已知科系名稱/關鍵字，代表使用者限定了特定科系，
        # 絕對不可啟動大範圍的 `is_career_planning` 模式！
        has_specified_dept = False
        dept_val = merged_filters.get("dept_short")
        if dept_val and dept_val != _EXPLICIT_NULL:
            has_specified_dept = True
        else:
            # 額外比對問題中是否有科系關鍵字，以防 LLM 沒提取出 dept_short 但問題其實寫了
            for k, info in config.DEPT_REGISTRY.items():
                short_name = info["short_name"]
                full_name = info["full_name"]
                candidates = [short_name, full_name] + info.get("aliases", []) + info.get("keywords", [])
                if any(cand in question for cand in candidates):
                    # 排除通識或體育等非專業科系
                    if short_name not in ("通識中心", "日大學體育"):
                        has_specified_dept = True
                        break
                        
        if has_specified_dept and is_career_planning:
            logger.info("  🛡️ 偵測到使用者指定了特定科系，強制將 is_career_planning 設為 False，防止誤入全校大搜索模式。")
            is_career_planning = False
        ...
        # Python 層級的學期鎖定或跨學期放行邏輯：
        # - 若問題涉及具體時間（如星期X、空堂、上下午、第X節等），我們強制鎖定當前真實學期，以防排課衝突。
        # - 若只是查詢學科主題或課程名稱（如「Linux相關課程」、「資料結構」），則自動放行至所有學期。
        has_time_constraint = any(kw in question for kw in ["星期", "禮拜", "週", "周", "今天", "明天", "空堂", "幾點", "下午", "上午", "節", "堂"])
        has_time_constraint = has_time_constraint or ("day_of_week" in merged_filters) or ("time_period" in merged_filters)
        
        if final_type in ("course_info", "schedule") and not is_career_planning:
            if has_time_constraint:
                if "academic_year" not in merged_filters or merged_filters["academic_year"] == _EXPLICIT_NULL:
                    merged_filters["academic_year"] = str(config.CURRENT_ACADEMIC_YEAR)
                if "semester" not in merged_filters or merged_filters["semester"] == _EXPLICIT_NULL:
                    merged_filters["semester"] = str(config.CURRENT_SEMESTER)
            else:
                # 【重要修復】只有在 LLM 沒有提取出明確學期，且沒有時間限制詞時，才將其放寬至所有學期。
                # 如果使用者明確指定了「113下學期」，必須尊重 LLM 提取的結果，不可強制清空。
                if "academic_year" not in merged_filters:
                    merged_filters["academic_year"] = _EXPLICIT_NULL
                if "semester" not in merged_filters:
                    merged_filters["semester"] = _EXPLICIT_NULL
                
                merged_filters["grade"] = _EXPLICIT_NULL
