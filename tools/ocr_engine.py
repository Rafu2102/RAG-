...
- 學年區：分為「第一學年(112)」、「第二學年(113)」、「第三學年(114)」等
- 每學年分為第一學期和第二學期兩欄
...
【你的任務】
1. 精確辨識學號、姓名、系所
2. 逐學期、逐課程提取：科目名稱、學分、成績、類別、修課狀態
3. 修課狀態判斷規則：
   - 成績 >= 60 且無特殊備註 → "及格"
   - 成績 < 60 → "不及格"
   - 備註為「停」→ "停修"
    ...
    for file_bytes, mime in zip(file_bytes_list, mime_types):
        if mime == "application/pdf":
            try:
                ...
                logger.info(f"📄 偵測到 PDF，使用 PyMuPDF 將 {len(doc)} 頁轉為 300DPI 圖片以提升表格辨識率...")
                for page_num in range(len(doc)):
                    page = doc.load_page(page_num)
                    # 放大 4.16 倍 (約等於 300 DPI)
                    mat = fitz.Matrix(4.16666, 4.16666)
                    pix = page.get_pixmap(matrix=mat)
                    processed_bytes_list.append(pix.tobytes("png"))
                    processed_mime_types.append("image/png")
            except ImportError:
                logger.warning("⚠️ 未安裝 PyMuPDF (fitz)，將使用效能較差的原生 PDF 辨識模式 (可能導致雜亂表格幻覺)")
