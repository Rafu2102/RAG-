...
def generate_answer(
    ...
    # ── 動態 thinkingLevel：根據意圖複雜度調整推理深度 ──
    # 簡單查詢（課程資訊、教授資訊）不需要深度思考，用 "medium" 加速回應
    # 複雜查詢（職涯規劃、跨課比較）才啟用 "high" 進行深度推理
    if is_career_planning:
        thinking_level = "high"
    elif intent in ("course_info", "professor_info", "policy_rules"):
